...
class DataPreprocessor:
    ...
    def preprocess_data(df: pd.DataFrame) -> pd.DataFrame:
        processed = df.copy()
        processed = processed[
            (processed['MARRIAGE'] != 0) & 
            (processed['EDUCATION'] != 0)
        ]
...
class MetricsCalculator:
    @staticmethod
    def compute_performance_metrics(dataset_name: str, y_true: np.ndarray, y_pred: np.ndarray) -> dict:
        return {
            "type": "metrics",
    ...
    @staticmethod
    def compute_confusion_matrix(dataset_name: str, y_true: np.ndarray, y_pred: np.ndarray) -> dict:
...
class CreditDefaultPredictor:
    ...
    def save_metrics(self, metrics_list: list, filepath: str):
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
